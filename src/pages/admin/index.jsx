...
export default function AdminPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        await load(); // 🔥 QUESTO È IL FIX IMPORTANTE
        setForm({ name: "", description: "", price: "", allergens: "" });
        setEditIndex(null);
      }
    } catch (err) {
        ...
        <div className="grid grid-cols-1">
              ...
              {!authOk ? (
                <form onSubmit={tryLogin} className="space-y-4">
                  <div>
                    ...
                    <input className="w-full px-3 py-2 rounded-md bg-white/5 border border-white/10 text-white outline-none"
                      value={user}
          ...
          <div className="col-span-2">
            {authOk ? (
              ...
              <div>Accedi per modificare il menu.</div>
            )}
          </div>
        </div>
      </div>
